                    ...
                    try {
						...
						provide_future = session->provide("com.examples.calculator.add2", &add2).then(
							...
							try {
								...
								prefix_provide_future = session->provide("com.examples.math", &math, { {"match", msgpack::object("prefix") } }).then(
									[&](boost::future<autobahn::wamp_registration> registration2) {
									try {
										std::cerr << "registered prefixed procedure:" << registration2.get().id() << std::endl;
