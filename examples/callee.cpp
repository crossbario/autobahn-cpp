    ...
    try {
        ...
        transport->connect([&]() {
            ...
            session->start([&]() {
                ...
                session->join([&](uint64_t joined) {
                    std::cerr << "joined realm: " << joined << std::endl;

                    session->provide([&](const autobahn::wamp_registration& registration) {
                        std::cerr << "registered procedure:" << registration.id() << std::endl;
                    },
                    on_exception,
                    "com.examples.calculator.add2",
                    &add2);
